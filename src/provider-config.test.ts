...
test("declares native API-key configuration without a management-command override", () => {
  ...
  const provider = manifest.contributes.languageModelChatProviders.find((item) => item.vendor === "aiand");
  assert.ok(provider);
...
test("keeps the legacy management commands available for the Secret Storage key", () => {
  ...
  assert.match(
    manifest.contributes.commands.find((item) => item.command === "aiandCopilot.testConnection")?.title ?? "",
    /Test Inference/,
  );
  assert.ok(manifest.contributes.commands.some((item) => item.command === "aiandCopilot.manage"));
  assert.match(
    manifest.contributes.commands.find((item) => item.command === "aiandCopilot.showUsage")?.title ?? "",
    /Credits and Usage/,
  );
});
