 ...
 *
 * Strictly opt-in via `aiandCopilot.inlineSuggestions`. The provider debounces
 * typing, aborts superseded in-flight requests, excludes the Copilot Chat
 * prompt box unless explicitly enabled, and returns a single completion item
 * whose insertText is the model's suggestion.
 */
...
export class AiandInlineCompletionProvider implements vscode.InlineCompletionItemProvider {
  ...
  provideInlineCompletionItems(
    ...
    // (users can opt in via aiandCopilot.inlineSuggestionsChatInput).
